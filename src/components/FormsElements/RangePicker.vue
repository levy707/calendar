<template>
  <div class="range-picker">
    ...
    <div class="range-picker__block" @click="openCalendar()">
      <input
        :class="['range-picker__input', { 'range-picker__input--selected': isFirstDatePicked }]"
      ...
      <input
        :class="['range-picker__input', { 'range-picker__input--selected': isSecondDatePicked }]"
        v-model="checkOutInput"
        placeholder="Check Out"
        type="text"
        :readonly="true"
      />
      <div v-if="isCalendarVisible" class="calendar" v-click-outside="closeCalendar">
        <div class="calendar__header">
          ...
          <div class=" calendar__header-title">{{ currentMonthName }} {{ currentYear }}</div>
          <button
        ...
        <div class="calendar__body">
          <div class="calendar__days">
            <div class="calendar__days-block" v-for="name in daysNames">
            ...
            <button
              ...
              :class="[
                'calendar__days-block',
                {
                  ...
                  'calendar__days-block--disabled': day.disabled || day.isTemporaryDisabled,
                },
              ]"
...
export default {
  ...
  computed: {
    ...
    currentMonthName() {
      return new Date(`${this.currentYear}, ${this.currentMonth + 1}`).toLocaleString('en', { month: 'long' });
    },
  ...
  watch: {
    currentMonth() {
      ...
      if (this.isAllDatesPicked) this.markRangedDays();
      if (this.firstBlockedDate) this.showOnlyAvailableCheckoutDates(this.checkInInput);
    },
    checkInInput(value) {
    ...
    isAllDatesPicked() {
      this.isAllDatesPicked ? this.markRangedDays() : '';
    },
    hoveredDay(hoveredDay) {
      this.currentMonthDays = this.currentMonthDays.map((day) => {
        day.isHoverActive = day.dateDay <= hoveredDay && day.dateDay >= new Date(this.checkInInput);
        return day;
      });
    },
  },
  ...
  methods: {
    ...
    pickDate(day) {
      ...
      } else if (this.isFirstDatePicked && !this.isSecondDatePicked) {
        if (new Date(this.getDateFormatToInput(day)).getTime() < new Date(this.checkInInput).getTime()) {
          this.initMonthArray();
          this.checkOutInput = '';
          this.checkInInput = this.getDateFormatToInput(day);
          this.showOnlyAvailableCheckoutDates(this.checkInInput);
    ...
    setFirstBlockedDate(pickedDate) {
      ...
      if (nextBlockedDates.length) this.firstBlockedDate = nextBlockedDates[0];
    },
    showOnlyAvailableCheckoutDates(pickedDate) {
      ...
      this.currentMonthDays = this.currentMonthDays.map((day) => {
        if (
          ...
          (!!this.firstBlockedDate && day.dateDay.getTime() > this.firstBlockedDate.getTime())
        ) {
    ...
    setHoveredDay(day) {
      if (
        ...
        new Date(this.checkInInput).getTime() < new Date(this.currentYear, this.currentMonth, day).getTime()
      )
        this.hoveredDay = new Date(this.currentYear, this.currentMonth, day);
    },
    clearHoveredDay() {
      if (this.isFirstDatePicked && !this.isSecondDatePicked) this.hoveredDay = '';
    },
