    ...
    <form class="booking-form">
        ...
        <div class="booking-form__grade">
            <div class="booking-form__grade-rating">
                <i
                    v-for="index in maxVoteNumber"
                    :key="`booking-form_${index}`"
                    :class="['icon', getVoteClass(index)]"
                ></i>
            </div>
            <div class="booking-form__grade-votes">{{ votes }}</div>
        </div>
        <RangePicker :blocked-dates="blockedDates" />
    </form>
</template>
...
export default {
    ...
    methods: {
        getVoteClass(currentValue) {
            ...
            if (Math.round(currentValue - this.rate) < 1) {
